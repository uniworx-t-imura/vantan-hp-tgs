...
function archive_scroll() {

    var _Sscroll = new SmoothScroll();
    var _headerH = 0;
    var options = {
        offset: 0, 
        speed: 1500,
    ...
    var smoothScrollWithoutHash = function (selector, settings) {
        ...
        var clickHandler = function (event) {
            ...
            if (!anchor) return;
            event.preventDefault();
            if (document.body.classList.contains('active') == true) {
                _dataAsClick = _scrollfromTop;
                document.body.classList.remove("active");
                  $('.menu-trigger').removeClass('active');
                _id_header.classList.add('-js_head-opacity');
                document.documentElement.style.height = "100%";
                document.body.style.top = '0px';
                window.scrollTo(0, _dataAsClick);
                // var headerHeight = 70;
            }
            _Sscroll.animateScroll(anchor, toggle, options );
            _state = false;
        };
